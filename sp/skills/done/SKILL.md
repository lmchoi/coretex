...
Pick the worktree under `.claude/worktrees/` being cleaned up (if more than one, ask the user which). Set `SLUG` to its directory name. Get `BRANCH` from git rather than reconstructing it — run this from the host repo, passing the worktree's absolute path:

```bash
<plugin>/scripts/sp-worktree branch-for /abs/path/to/.claude/worktrees/<slug>
```
...
Run the plugin's `scripts/sp-preflight` by absolute path, from the host repo for the `plans` directory and the optional `todo` file.
