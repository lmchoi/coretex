...
Run the plugin's `scripts/sp-preflight --gh` by absolute path, from the host repo for the `plans` directory and `test` command, and to confirm the GitHub CLI is authenticated before starting work that ends in a push. Read `<plans>/<slug>.md` for the current branch.
